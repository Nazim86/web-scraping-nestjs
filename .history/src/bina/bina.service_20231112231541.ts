  ...
  async getHouses(price) {
    ...
    try {
      ...
      while (nextPage == 'Növbəti') {
        ...
        const x = jschardet.detect(stringToCheck);
...
const utf8String1 = iconv.decode(iconv.encode(string1Windows1252, 'utf-8'), 'utf-8');
const utf8String2 = iconv.decode(iconv.encode(string2Windows1252, 'utf-8'), 'utf-8');

console.log(utf8String1 === utf8String2);
This example uses iconv.encode() to encode the strings from Windows-1252 to UTF-8 and then uses iconv.decode() to convert them back to JavaScript strings. The final comparison should now correctly reflect whether the strings are equal.

Remember to replace the placeholder strings with your actual data. If this resolves the issue, it indicates that the original strings were indeed encoded in Windows-1252, and the conversion to UTF-8 has made them comparable.

Keep in mind that loss of information can occur during encoding conversion if the source encoding doesn't support certain characters present in the target encoding.
